            ...
            if '-------' in y and 'Custom' in (objectgroups[objectgroups.index(y)+2]) and (objectgroups[objectgroups.index(y)+4]) != '':
                ...
                while i > 0:
                    ...
                    objectlist += [objectmember.strip().replace('(', '_').replace(
                        ')', '_').replace('/', '_').replace('\\', '_').replace(' ', '_').replace('*', '_').replace(',', '_').replace('+', '_').replace('"', '_').replace('&', '_')]
                    i += 1
                    if objectgroups[i] == '':
                        extractgroupdict = {}
                        objectgroupdict.update({objectgroupname.strip().replace('(', '_').replace(
                            ')', '_').replace('/', '_').replace('\\', '_').replace(' ', '_').replace('*', '_').replace(',', '_').replace('+', '_').replace('"', '_').replace('&', '_'): objectlist})
        ...
        if '-------' in a and 'member' in (servicegroups[servicegroups.index(a)+3]):
            ...
            while i > 0:
                ...
                if objectmember.strip().replace('(', '_').replace(
                        ')', '_').replace('/', '_').replace('\\', '_').replace(' ', '_').replace('*', '_').replace(',', '_').replace('+', '_').replace('"', '_').replace('&', '_') not in extractserviceobject.list:
